...
def build(bld):
    ...
    # Copy other data files to build bundle (build/guitarSynth.lv2)
    for i in ['guitarSynth.ttl']:
        bld(features     = 'subst',
            is_copy      = True,
            source       = i,
    ...
    obj = bld(features     = 'c cshlib lv2lib',
              source       = 'guitarSynth.c',
              name         = 'guitarSynth',
              target       = 'lv2/%s/guitarSynth' % bundle,
              install_path = '${LV2DIR}/%s' % bundle,
              uselib       = 'M LV2')
